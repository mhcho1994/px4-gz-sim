...
cat << EOF >> "${tmpfile}"
${MARK_BEGIN}
# Auto-generated by usersetup.sh - source this block to set up the flightstack_sim environment
# This block is managed by usersetup.sh - do not edit manually
# To update this block, edit and re-run usersetup.sh
...
# --------------------------
# PX4 ROS 2 plugin environment (optional)
# If the corresponding workspace install/setup.bash exists, source it.
# --------------------------
PX4_ROS_UXRCE_DDS_ENV="${PROJECT_ROOT}/ros2/px4_ros_uxrce_dds_ws"
if [ -f "${PX4_ROS_UXRCE_DDS_ENV}/install/setup.bash" ]; then
  source "${PX4_ROS_UXRCE_DDS_ENV}/install/setup.bash"
fi

PX4_MSGS_ENV="${PROJECT_ROOT}/ros2/px4_msgs_ws"
if [ -f "${PX4_MSGS_ENV}/install/setup.bash" ]; then
  source "${PX4_MSGS_ENV}/install/setup.bash"
fi
