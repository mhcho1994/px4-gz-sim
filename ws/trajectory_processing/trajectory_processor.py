...
def resample_and_extract_features(t, x, y, z, vx, vy, vz, *, window_len=200, poly_order=3):
    """
    ...
    x_new = interp1d(t, x, bounds_error=True, fill_value=(x[0], x[-1]))(t_new)
    y_new = interp1d(t, y, bounds_error=True, fill_value=(y[0], y[-1]))(t_new)
    z_new = interp1d(t, z, bounds_error=True, fill_value=(z[0], z[-1]))(t_new)
    vx_new = interp1d(t, vx, bounds_error=True, fill_value=(vx[0], vx[-1]))(t_new)
    vy_new = interp1d(t, vy, bounds_error=True, fill_value=(vy[0], vy[-1]))(t_new)
    vz_new = interp1d(t, vz, bounds_error=True, fill_value=(vz[0], vz[-1]))(t_new)
...
def process_px4_flight_data(ulog_path):
    try:
        ...
        extracted = resample_and_extract_features(t_loc, x, y, z, vx, vy, vz)
...
def collect_all_runs_segment_dataframe(base_data_dir=Path("./data/sitl_logs")):
    """
...
if __name__ == "__main__":
    ...
    data_processed = collect_all_runs_segment_dataframe(base_data_dir = BASE_DATA_DIR)